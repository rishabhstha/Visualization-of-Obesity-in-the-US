...
income_race<-data%>%
  ...
  rename("American Indian/Alaska Native"="aian_est")%>%
  rename("Asian"="asian_est")%>%
  rename("Non-Hispanic Black"="black_est")%>%
  rename("Hispanic"="hisp_est")%>%
  rename("Non-Hispanic White"="white_est")%>%
  rename("Hawaiian/PacificIslander"="nhopi_est")%>%
  select(-"Total Pop Income")%>%
  pivot_longer("American Indian/Alaska Native":"Non-Hispanic White",names_to="Race",values_to="Median Household Income")%>%
  na.omit() %>% 
  group_by(Race,year) %>% 
  summarize(average_median_income=mean(`Median Household Income`))
  
color<-c("blue","orange","cyan","green","pink","brown")
...
income_race%>%
  ...
  geom_bar(mapping=aes(x=reorder(Race,average_median_income)),stat='identity')+
  facet_wrap(~year)+
  
  theme(axis.text.x=element_blank(),plot.title = element_text(hjust = 0.5))+
  labs(title="Average Household Median Income by Race",y="Average Median Income")
